...
- `safety_close_time: int` — timeout for forcing position close (seconds)
...
- `get_action_amount(price: float) -> float` — returns `coinUse.action_amount` or `coinGet.action_amount` depending on direction
- `get_target() -> float` — returns next unexecuted price target from `price_close`
- `is_stop_loss_triggered(cur_price: float) -> bool` — abstract; LongPosition returns `cur_price <= price_stop_loss`
...
- `finalize()` MUST reset all state — `price_open`, `price_close`, `executed_open`, `executed_close`, `coinUse.reset()`, `coinGet.reset()`, `state = POSITION_STATE_WAIT`, `action = STRATEGY_ACTION_NOTHING`
