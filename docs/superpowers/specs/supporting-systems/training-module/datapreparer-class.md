...
- **NN result columns are not produced or merged here.** `DataPreparer` produces NN **feature** (`{tf}_nn_*`) and **target** (`1_nn_tgt_*`) columns, but never NN **result** columns (`nn_res_*`). Those come from `NNOrchestrator.run_inference()` as a separate `df_with_nn.pkl` and are joined into the wide view by consumers (`SimulationData`/`FullData`/`LiveData`) at load time — see `nnpredictor-class.md` §3 and `data-class.md` §2. `DataPreparer` stays the **single writer** of `df_with_indicators.pkl`, model-free, and base-image only (no torch / checkpoint dependency). Running inference does not require re-running this expensive `prepare()` pass.
